...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    if (client.connect("TravisBraydonTyler")) {
...
void loop() {
  ...
  if(!client.loop())
    client.connect("TravisBraydonTyler");
  
  //Publish the sensor data to the associated topics
  client.publish("4ID3_GroupF/Temperature", String(dhtTempEvent.temperature).c_str());
  delay(100);
  client.publish("4ID3_GroupF/humidity", String(dhtHumEvent.relative_humidity).c_str());
  delay(100);
  client.publish("4ID3_GroupF/pressure", String(bmpEvent.pressure).c_str());
  delay(100);
  client.publish("4ID3_GroupF/light", String(lux).c_str());
